...
Action = Enum('Action', ['RESETUD', 'ADDUSER', 'DELUSER', 'MODUSER', 'LIST'])
...
class MuMgr(object):
    def __init__(self):
        ...
        try:
            ...
            logging.error(e.message + 'set server_addr to 127.0.0.1')
    ...
    def add(self, user):
        up = {'enable': 1, 'u': 0, 'd': 0, 'method': "aes-128-ctr", 'protocol': "auth_aes128_md5",
              'obfs': "tls1.2_ticket_auth_compatible", 'transfer_enable': 9007199254740992, 'passwd': self.rand_pass()}
        up.update(user)
    ...
    try:
        ...
        for key, value in optlist:
            if key == '-a':
                ...
                action = Action.DELUSER
            elif key == '-e':
                action = Action.MODUSER
            elif key == '-l':
                action = Action.LIST
            elif key == '-c':
                action = Action.RESETUD
            elif key == '-u':
                user['user'] = value
            elif key == '-i':
                user['muid'] = int(value)
